...
public class CartServiceImpl implements CartService {
    ...
    public void initialiseCartList() {
        ...
        cart1TotalAmount= cartItemList.stream().map( p->p.getProduct().getPrice().multiply(BigDecimal.valueOf(p.getQuantity())))
...
//        }
        ...
        // todo change to stream
        cart2TotalAmount= cartItemList1.stream().map( p->p.getProduct().getPrice().multiply(BigDecimal.valueOf(p.getQuantity())))
                .reduce(BigDecimal.ZERO, BigDecimal::add);
